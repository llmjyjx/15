...
async function load(){
  ...
  const main=resolveAssetUrl(d.main_image||(d.images?.[0]?.url||''));
  detail.innerHTML=`<div class="detail-top"><div class="eyebrow">全部作品 / 作品详情</div><h1>${esc(d.name)}</h1></div><div class="detail-layout"><div><div class="detail-main-image">${main?`<img id="mainImg" src="${esc(main)}" alt="${esc(d.name)}">`:''}</div><div class="thumbs">${(d.images||[]).map(x=>`<img class="${x.is_main?'active':''}" src="${esc(resolveAssetUrl(x.url))}" alt="" onclick="showImg('${esc(x.url)}',this)">`).join('')}</div></div><aside><div class="meta"><div class="meta-row"><b>作品分类</b><span>${esc(d.category||'')}${d.subcategory?' / '+esc(d.subcategory):''}</span></div><div class="meta-row"><b>作品编号</b><span>${esc(d.work_no||'')}</span></div><div class="meta-row"><b>创作时间</b><span>${esc(d.creation_time||'')}</span></div><div class="meta-row"><b>尺寸</b><span>${esc(d.size||'')}</span></div></div><div class="description">${d.description||''}</div></aside></div>`
}
