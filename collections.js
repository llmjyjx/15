...
function load(){const q=new URLSearchParams({page:state.page,size:state.size,keyword:state.keyword,category:state.category,subcategory:state.subcategory,period:state.period});apiFetch('api/works?'+q).then(r=>r.json()).then(d=>{count.textContent=`共 ${d.total} 件作品`;const hint=document.getElementById('pageSizeHint');if(hint)hint.textContent=`每页 ${state.size} 件`;grid.innerHTML=d.items.length?d.items.map(x=>`<article class="work-card" onclick="location='detail.html?id=${x.id}'"><div class="pic" style="position:relative;width:100%;aspect-ratio:1/1;overflow:hidden;background:#ebe9e3;margin-bottom:12px"><img src="${esc(resolveAssetUrl(x.main_image))}" alt="${esc(x.name||'')}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center;display:block"></div><h3>${esc(x.name)}</h3><p>${esc(x.category||'')}${x.subcategory?' · '+esc(x.subcategory):''}</p><p>${esc(x.work_no||'')}　${esc(x.creation_time||'')}</p></article>`).join(''):'<div class="empty">暂无符合条件的作品</div>';pages.innerHTML=renderPager(d.page,d.pages,'state.page=__P__;load()')}).catch(()=>{grid.innerHTML='<div class="empty">暂时无法读取作品</div>'})}
