...
function load(){const q=new URLSearchParams({page:state.page,size:20,keyword:state.keyword,category:state.category,subcategory:state.subcategory,period:state.period});apiFetch('api/works?'+q).then(r=>r.json()).then(d=>{count.textContent=`共 ${d.total} 件作品`;grid.innerHTML=d.items.length?d.items.map(x=>`<article class="work-card" onclick="location='detail.html?id=${x.id}'"><div class="pic"><img src="${esc(resolveAssetUrl(x.main_image))}" alt="${esc(x.name||'')}"></div><h3>${esc(x.name)}</h3><p>${esc(x.category||'')}${x.subcategory?' · '+esc(x.subcategory):''}</p><p>${esc(x.work_no||'')}　${esc(x.creation_time||'')}</p></article>`).join(''):'<div class="empty">暂无符合条件的作品</div>';pages.innerHTML=Array.from({length:d.pages},(_,i)=>`<button type="button" class="${i+1===d.page?'active':''}" onclick="state.page=${i+1};load()">${i+1}</button>`).join('')}).catch(()=>{grid.innerHTML='<div class="empty">暂时无法读取作品</div>'})}
