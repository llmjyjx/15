...
let worksPageSize=Number(localStorage.getItem('xiangpan_works_page_size')||20);if(![15,20,25,30,35,40,45,50].includes(worksPageSize))worksPageSize=20;
...
loginBtn.onclick=async()=>{try{const d=await jsonFetch('api/admin/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({username:u.value,password:p.value})});show(d)}catch(e){alert(e.message)}};
...
async function works(page=1){
  ...
  <div class="bulk-toolbar"><label><input id="selectAllWorks" class="bulk-check" type="checkbox" onchange="toggleAllWorks(this)"> 全选本页</label><button class="danger" type="button" onclick="batchDeleteWorks()">批量删除</button><span class="hint">每页最多选择20件</span></div>
  <table class="table works-table"><thead><tr><th class="check-col"></th><th class="col-thumb">主图</th><th class="col-name">作品名称</th><th class="col-no">作品编号</th><th class="col-cat">分类</th><th class="col-time">创作时间</th><th class="col-size">尺寸</th><th class="col-actions">操作</th></tr></thead><tbody>${d.items.map(x=>`<tr><td class="check-col"><input class="bulk-check work-check" type="checkbox" value="${x.id}"></td><td class="col-thumb"><img class="list-thumb" src="${esc(x.main_image||'img/avatar.webp')}" alt="" title="${esc(x.name)}"></td><td class="col-name" title="${esc(x.name)}">${esc(x.name)}</td><td class="col-no" title="${esc(x.work_no||'')}">${esc(x.work_no||'')}</td><td class="col-cat" title="${esc(x.category||'')}${x.subcategory?' / '+esc(x.subcategory):''}">${esc(x.category||'')}${x.subcategory?' / '+esc(x.subcategory):''}</td><td class="col-time">${esc(x.creation_time||'')}</td><td class="col-size" title="${esc(x.size||'')}">${esc(x.size||'')}</td><td class="col-actions actions"><button type="button" onclick="editWork(${x.id})">编辑</button><button type="button" onclick="delWork(${x.id})">删除</button></td></tr>`).join('')}</tbody></table>
  ...
  <label class="page-size-label">每页 <select id="worksPageSize" onchange="setWorksPageSize(this.value)">${[15,20,25,30,35,40,45,50].map(n=>`<option value="${n}" ${n===worksPageSize?'selected':''}>${n}</option>`).join('')}</select> 条</label></div>`;
...
async function batchImportWorks(){
  ...
  <div class="form-box work-form-box">
    ...
    · 文件名含年份（如 <code>1999国画山水.jpg</code>）：作品名称=1999国画山水，创作时间=1999<br>
...
async function doBatchImport(){
  ...
  wrap.style.display='block'; bar.style.width='0%'; prog.textContent=`准备上传 ${files.length} 张图片…`; btn.disabled=true;
  try{
...
async function formWork(d){if(workEditor){try{workEditor.destroy()}catch(e){}workEditor=null}cats=await jsonFetch('api/categories');
 const images=(d.images||[]).map(img=>`<div class="image-item"><img src="${esc(img.url)}" alt=""><div><button onclick="setMain(${d.id},${img.id})">${img.is_main?'主图':'设为主图'}</button><button onclick="removeImage(${d.id},${img.id})">删除</button></div></div>`).join('');
...
async function siteView(){if(bioEditor){try{bioEditor.destroy()}catch(e){}bioEditor=null}const d=await jsonFetch('api/site');let social={微博:'',抖音:'',公众号:''};try{const raw=JSON.parse(d.social_links||'{}');if(Array.isArray(raw)){social.微博=raw[0]||'';social.抖音=raw[1]||'';social.公众号=raw[2]||''}else if(raw&&typeof raw==='object'){social.微博=raw.微博||'';social.抖音=raw.抖音||'';social.公众号=raw.公众号||''}}catch(e){}view.innerHTML=`<h1 class="admin-title">站点管理</h1><div class="form-box site-form-box"><div class="field"><label>站点名称 <i>必填，最多20字</i></label><input id="sn" maxlength="20" value="${esc(d.site_name||'')}"></div><div class="field"><label>站点标志（建议400×300，不限制尺寸）</label><div class="upload-line"><img id="logoPreview" class="site-preview" src="${esc(resolveAssetUrl(d.logo||'img/avatar.webp'))}"><input id="sl" type="file" accept="image/*"></div></div><div class="field"><label>艺术家头像（建议200×200，不限制尺寸）</label><div class="upload-line"><img id="avatarPreview" class="site-preview" src="${esc(resolveAssetUrl(d.avatar||'img/avatar.webp'))}"><input id="sa" type="file" accept="image/*"></div></div><div class="field"><label>艺术家姓名 <i>最多20字</i></label><input id="san" maxlength="20" value="${esc(d.artist_name||'')}"></div><div class="field"><label>艺术家简介</label><div class="wang-editor-wrap"><div id="bio-toolbar" class="wang-toolbar"></div><div id="bio-editor" class="wang-content"></div></div></div><div class="field"><label>社交媒体</label><div class="social-admin-grid"><label>微博<input id="socialWeibo" type="url" placeholder="微博链接" value="${esc(social.微博)}"></label><label>抖音<input id="socialDouyin" type="url" placeholder="抖音链接" value="${esc(social.抖音)}"></label><label>公众号<input id="socialWechat" type="url" placeholder="公众号链接" value="${esc(social.公众号)}"></label></div><small class="hint">每项填写完整的 http:// 或 https:// 链接；留空的项目不会显示在网站底部。</small></div><div class="form-actions"><button class="primary" onclick="saveSite()">保存</button></div></div>`;bioEditor=initEditor('#bio-editor','#bio-toolbar',d.artist_bio||'');document.getElementById('sl').onchange=e=>previewSiteImage(e,'logoPreview');document.getElementById('sa').onchange=e=>previewSiteImage(e,'avatarPreview')}
function previewSiteImage(e,id){const f=e.target.files?.[0];if(f)document.getElementById(id).src=URL.createObjectURL(f)}
async function saveSite(){const siteName=document.getElementById('sn').value.trim(),artistName=document.getElementById('san').value.trim();if(!siteName){alert('站点名称不能为空');return}if([...siteName].length>20||[...artistName].length>20){alert('名称最多20个字');return}const social={微博:document.getElementById('socialWeibo').value.trim(),抖音:document.getElementById('socialDouyin').value.trim(),公众号:document.getElementById('socialWechat').value.trim()},re=/^https?:\/\/[^\s]+$/i;if(Object.values(social).some(x=>x&&!re.test(x))){alert('社交媒体链接格式不正确，请填写 http:// 或 https:// 链接');return}const f=new FormData();f.append('site_name',siteName);f.append('artist_name',artistName);f.append('artist_bio',bioEditor?bioEditor.getHtml():'');f.append('social_links',JSON.stringify(social));const li=document.getElementById('sl'),ai=document.getElementById('sa');if(li.files[0])f.append('logo',li.files[0]);if(ai.files[0])f.append('avatar',ai.files[0]);try{await jsonFetch('api/admin/site',{method:'POST',body:f});alert('已保存');siteView()}catch(e){alert(e.message)}}
