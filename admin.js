...
async function works(page=1){
  ...
  <table class="table works-table"><thead><tr><th class="check-col"></th><th class="col-thumb">主图</th><th class="col-name">作品名称</th><th class="col-no">作品编号</th><th class="col-cat">分类</th><th class="col-time">创作时间</th><th class="col-size">尺寸</th><th class="col-actions">操作</th></tr></thead><tbody>${d.items.map(x=>`<tr><td class="check-col"><input class="bulk-check work-check" type="checkbox" value="${x.id}"></td><td class="col-thumb"><img class="list-thumb" src="${esc(x.main_image||'')}" alt="" title="${esc(x.name)}"></td><td class="col-name" title="${esc(x.name)}">${esc(x.name)}</td><td class="col-no" title="${esc(x.work_no||'')}">${esc(x.work_no||'')}</td><td class="col-cat" title="${esc(x.category||'')}${x.subcategory?' / '+esc(x.subcategory):''}">${esc(x.category||'')}${x.subcategory?' / '+esc(x.subcategory):''}</td><td class="col-time">${esc(x.creation_time||'')}</td><td class="col-size" title="${esc(x.size||'')}">${esc(x.size||'')}</td><td class="col-actions actions"><button type="button" onclick="editWork(${x.id})">编辑</button><button type="button" onclick="delWork(${x.id})">删除</button></td></tr>`).join('')}</tbody></table>
